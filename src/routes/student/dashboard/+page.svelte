<script>
  ...
  let count = data.tests.length; // Total number of tests
  let perPage;
  let siblingCount;
...
<nav class="bg-gray-800 p-4">
  <div class="container mx-auto flex justify-between items-center">
    <div class="space-x-4">
      <a href="/student/logout" class="text-white hover:bg-gray-700 px-4 py-2 rounded transition-colors">Logout</a>
    </div>
  </div>
</nav>

<main class="container mx-auto p-4">
  <h1 class="text-3xl md:text-4xl font-bold mb-6 text-center">Student Dashboard</h1>
  ...
  <div class="bg-white p-6 rounded-lg shadow-lg">
    ...
    {:else}
      ...
      <Table.Root>
        <Table.Caption>A list of your available tests.</Table.Caption>
        <Table.Header>
          <Table.Row>
            <Table.Head class="w-[100px]">Test ID</Table.Head>
            <Table.Head>Title</Table.Head>
            <Table.Head class="text-right">Take Test</Table.Head>
          </Table.Row>
        </Table.Header>
        <Table.Body>
          {#each paginatedTests as test}
            <Table.Row>
              ...
              <Table.Cell class="text-right">
                <a target="_blank" href={`/student/test/${test.id}`} class="text-blue-500 hover:underline">Take Test</a>
              </Table.Cell>
            </Table.Row>
          {/each}
        </Table.Body>
